...
export default function Layout() {
  ...
  const setMenu = (name: string, val: boolean) =>
    setOpenMenus((prev) => ({ ...prev, [name]: val }));
  ...
  const navigation = [
    ...
    },
    {
      ...
      icon: BookOpen,
      submenu: [
        { name: 'Học từ vựng', href: '/vocabulary/beginner' },
        { name: 'Danh sách từ vựng', href: '/vocabulary-list' },
      ],
    },
    {
      name: 'Ngữ pháp',
      icon: GraduationCap,
      submenu: [
        { name: 'Học ngữ pháp', href: '/grammar/beginner' },
        { name: 'Danh sách ngữ pháp', href: '/grammar-list' },
      ],
    },
    { name: 'Luyện Đọc', href: '/reading', icon: BookMarked },
    {
      name: 'Luyện tập',
      icon: FileText,
      submenu: [
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <nav className="hidden md:flex items-center space-x-0.5">
              {navigation.map((item) => {
                ...
                if (item.submenu) {
                  const isActive = item.submenu.some((sub) => location.pathname.startsWith(`/${sub.href.split('/')[1]}`));
                  const menuOpen = !!openMenus[item.name];

                  return (
                    <div key={item.name} className="relative">
                      <button
                        onMouseEnter={() => setMenu(item.name, true)}
                        onMouseLeave={() => setMenu(item.name, false)}
                      ...
                      {menuOpen && (
                        <div
                          onMouseEnter={() => setMenu(item.name, true)}
                          onMouseLeave={() => setMenu(item.name, false)}
                          className="absolute left-0 mt-1 w-52 bg-white rounded-md shadow-lg border border-gray-200 py-1 z-50"
                        >
